...
class Purchase extends Model
{
    ...
    )
    {
        return DB::transaction(function () use (
            ...
            $rePcsLSC = PurchaseLog::stockChange($id, null, Event::purchase()->value, $id, LogEventFeature::pcs_add()->value, null, null, null, $purchase_user_id, $purchase_user_name);
...
//            dd($purchase->isDirty(), $purchase);
            if ($purchase->isDirty()) {
                foreach ($purchase->getDirty() as $key => $val) {
                    ...
                    $rePcsLSC = PurchaseLog::stockChange($id, null, Event::purchase()->value, $id, LogEventFeature::pcs_change_data()->value, null, null, $event, $operator_user_id, $operator_user_name);
                    if ($rePcsLSC['success'] == 0) {
                        DB::rollBack();
                        return $rePcsLSC;
                    }
...
//                        "supplier_id" => $purchaseReq['supplier'],
    ...
    public static function del($id, $operator_user_id, $operator_user_name) {
        ...
        if (null != $inbounds && 0 < count($inbounds)) {
            ...
            return DB::transaction(function () use ($id, $operator_user_id, $operator_user_name
                ...
                $rePcsLSC = PurchaseLog::stockChange($id, null, Event::purchase()->value, $id, LogEventFeature::pcs_del()->value, null, null, null, $operator_user_id, $operator_user_name);
                if ($rePcsLSC['success'] == 0) {
                    DB::rollBack();
                    return $rePcsLSC;
                }
    ...
    public static function close($id, $operator_user_id, $operator_user_name) {
        ...
        PurchaseLog::stockChange($id, null, Event::purchase()->value, $id, LogEventFeature::pcs_close()->value, null, null, null, $operator_user_id, $operator_user_name);
    }
