...
class OnePage extends Model
{
    ...
    public static function getProducts($collection_id, $sale_channel_id)
    {
        ...
        $dataList = DB::table('prd_products as product')
            ...
            ->selectRaw('IF(product.feature IS NULL,"",product.feature) as _feature')
            ->selectRaw('IF(product.logistic_desc IS NULL,"",product.logistic_desc) as _logistic_desc')
            ->selectRaw('IF(product.slogan IS NULL,"",product.slogan) as _slogan')
