...
class PayingOrder extends Model
{
    ...
    public static function paying_order_list(
        ...
        $query = DB::table(DB::raw('(
                ...
                GROUP BY source_type, source_id, source_sub_id, (CASE WHEN source_type != "pcs_purchase" THEN type END)' . $separate . '
                ) AS po')
            )
            ->leftJoin(DB::raw('(
                SELECT pay_order_id,
            ...
            // ord_received_orders - refund
            ->leftJoin('ord_received_orders AS ro', function ($join) {
                $join->on('po.source_id', '=', 'ro.id');
                $join->where([
                    'po.source_type'=>app(ReceivedOrder::class)->getTable(),
                    'po.source_sub_id'=>null,
                    'po.type'=>9,
                    'ro.deleted_at'=>null,
                ]);
            ...
            ->leftJoin(DB::raw('(
                SELECT
                    refund_item.source_ro_id,
                    refund_item.append_po_id,
                    CONCAT(\'[\', GROUP_CONCAT(\'{
                            "product_owner":"\', "", \'",
                            "title":"\', refund_item.title, \'",
                            "sku":"\', "", \'",
                            "all_grades_id":"\', refund_item.grade_id, \'",
                            "grade_code":"\', COALESCE(refund_item.grade_code, ""), \'",
                ...
                ) AS refund_items_table'), function ($join){
                    $join->on('ro.id', '=', 'refund_items_table.source_ro_id');
                    $join->on('po.id', '=', 'refund_items_table.append_po_id');
            })

            // sub order out
            ->leftJoin('dlv_delivery AS sub_order_out', function ($join) {
                $join->on('po.source_id', '=', 'sub_order_out.id');
                $join->where([
                    'po.source_type'=>app(Delivery::class)->getTable(),
                    'po.source_sub_id'=>null,
                    'po.type'=>8,
            ...
            ->leftJoin(DB::raw('(
                SELECT
                    dlv_out_stock.delivery_id,
                    CONCAT(\'[\', GROUP_CONCAT(\'{
                        "product_owner":"\', COALESCE(p_owner.name, ""), \'",
                        "title":"\', dlv_out_stock.product_title, \'",
                        "sku":"\', dlv_out_stock.sku, \'",
                        "all_grades_id":"\', COALESCE(grade.id, ""), \'",
                        "grade_code":"\', COALESCE(grade.code, ""), \'",
                        "grade_name":"\', COALESCE(grade.name, ""), \'",
                ...
                LEFT JOIN prd_product_styles ON prd_product_styles.id = dlv_out_stock.product_style_id
                LEFT JOIN prd_products AS product ON product.id = prd_product_styles.product_id
                LEFT JOIN usr_users AS p_owner ON p_owner.id = product.user_id
                WHERE (product.deleted_at IS NULL AND dlv_out_stock.qty > 0 AND dlv_out_stock.show = 1)
                GROUP BY dlv_out_stock.delivery_id
                ) AS delivery_out'), function ($join){
                    $join->on('delivery_out.delivery_id', '=', 'sub_order_out.id');
            })
            ...
            ->selectRaw('
                CASE
                    ...
                    WHEN po.source_type = "' . app(Delivery::class)->getTable() . '" AND po.type = 8 THEN sub_order_out.event_sn
                    ELSE NULL
                END AS source_sn
            ')
            ...
            ->selectRaw('
                CASE
                    ...
                    WHEN po.source_type = "' . app(Delivery::class)->getTable() . '" AND po.type = 8 THEN delivery_out.items
                    ELSE NULL
            ...
            ->selectRaw('
                CASE
                    ...
                    WHEN po.source_type = "' . app(Delivery::class)->getTable() . '" AND po.type = 8 THEN NULL
                    ELSE NULL
        ...
        if ($source_sn) {
            $query->where(function ($query) use ($source_sn) {
                $query->where('purchase.sn', 'like', "%{$source_sn}%")
                    ...
                    ->orWhere('ro.sn', 'like', "%{$source_sn}%")
                    ->orWhere('sub_order_out.event_sn', 'like', "%{$source_sn}%");
            });
        }
    ...
    public static function paying_order_link($source_type, $source_id, $source_sub_id = null, $type)
    {
        ...
        } else if($source_type == 'ord_received_orders' && $type == 9){
            $link = route('cms.collection_payment.refund-po-show', ['id' => $source_id]);
        }
    ...
    public static function paying_order_source_link($source_type, $source_id, $source_sub_id = null, $type, $back_domain = false)
    {
        ...
        if($source_type == 'pcs_purchase'){
            ...
            $ro = ReceivedOrder::find($source_id);
            $link = ReceivedOrder::received_order_link($ro->source_type, $ro->source_id);
            // $link = ReceivedOrder::received_order_source_link($ro->source_type, $ro->source_id);
        }
