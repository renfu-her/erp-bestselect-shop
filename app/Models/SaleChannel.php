...
class SaleChannel extends Model
{
    ...
    /**
     * 同步單一款式在所有通路的價錢， 按下去所有通路都同步官網的售價/經銷價/定價/獎金/鴻利點數
     * @param $styleId 該產品款式ID
     * @return void
     */
    public static function batchProductStylePrice($styleId)
    {
        DB::beginTransaction();
        ...
        //購物官網通路款式價錢
        $officialStyleProductCount = DB::table('prd_salechannel_style_price')
            ->where('sale_channel_id', $officialSalesChannelId->id)
            ->where('style_id', $styleId)
            ->count();

        if ($officialStyleProductCount == 1) {
            $officialStyleProduct = DB::table('prd_salechannel_style_price')
                ->where('sale_channel_id', $officialSalesChannelId->id)
                ->where('style_id', $styleId)
                ->get()
                ->first();
            foreach ($otherSalesChannels as $otherSalesChannel) {
                ...
                if (!$newPrice) {
                    DB::table('prd_salechannel_style_price')
                        ->insert([
                            'style_id'        => $officialStyleProduct->style_id,
                            'sale_channel_id' => $otherSalesChannel->id,
                            'dealer_price'    => $officialStyleProduct->dealer_price,
                            'origin_price'    => $officialStyleProduct->origin_price,
                            'price'           => $officialStyleProduct->price,
                            'bonus'           => $officialStyleProduct->bonus,
                            'dividend'        => $officialStyleProduct->dividend,
                        ]);
                } else {
                    DB::table('prd_salechannel_style_price')
                        ->where([
