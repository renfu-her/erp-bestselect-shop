...
class CreateAccStituteOrderItemsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('acc_stitute_orders', function (Blueprint $table) {
            $table->dropColumn('qty');
            $table->dropColumn('total_price');
            $table->dropColumn('tw_dollar');
            $table->dropColumn('rate');
            $table->dropColumn('currency_id');
            $table->dropColumn('stitute_grade_id');
            $table->dropColumn('summary');
            $table->dropColumn('memo');
            $table->dropColumn('taxation');
        });

        Schema::create('acc_stitute_order_items', function (Blueprint $table) {
            $table->id();
            $table->integer('stitute_order_id')->comment('代墊單id');
    ...
    {
        Schema::dropIfExists('acc_stitute_order_items');
        ...
        Schema::table('acc_stitute_orders', function (Blueprint $table) {
            $table->after('sn', function ($tb) {
                ...
                $tb->unsignedBigInteger('stitute_grade_id')->comment('代墊單會計科目id');
