    ...
    public function up()
    {
        Schema::table('prd_sale_channels', function (Blueprint $table) {
            $table->after('discount', function ($tb) {
                $tb->integer('dividend_limit')->default(0)->comment('紅利上限');
                $tb->integer('dividend_rate')->default(0)->comment('紅利回饋比例');
                $tb->integer('event_dividend_rate')->default(0)->comment('活動紅利回饋比例');
