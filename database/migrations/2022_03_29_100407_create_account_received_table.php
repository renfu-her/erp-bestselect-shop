    ...
    public function up()
    {
        Schema::create('acc_received', function (Blueprint $table) {
            $table->id()->comment('收款管理：1.連結不同的「收款單類型」、收款單ID
                                                        ...
                                                        3.儲存不同收款方式中的共同欄位');
            $table->string('received_type')->comment('收款單類型,，例如:訂單');
            $table->unsignedBigInteger('received_type_id')->comment('不同「收款類型」對應到不同收款單table的 primary ID');

            $table->string('received_method')->comment('收款方式(支票、信用卡、匯款、外幣、現金、應收帳款、其它)對應的model class name');
            $table->unsignedBigInteger('received_method_id')
                ->nullable()
                ->comment('對應收款方式(支票、信用卡、匯款、外幣)table的primary id。註：現金、應收帳款、其它，不需對應到其它table，為null');

            $table->unsignedBigInteger('all_grades_id')->comment('收款會計科目');

            $table->decimal('tw_price')->comment('金額(新台幣)');
//            $table->dateTime('deposit_date')->comment('入帳日期');
