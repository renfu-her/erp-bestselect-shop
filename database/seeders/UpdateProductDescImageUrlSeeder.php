...
{
    /**
     * 把產品描述的圖片連結從edgio-cdn-pix.besttour.com.tw改成images-besttour.cdn.hinet.net
     * @return void
     */
    public function run()
    {
        ...
        foreach ($productData as $prdKey => $productDatum) {
            if ($productDatum->desc){
                ...
                $descImages = $descHTML->find('img[src^="https://edgio-cdn-pix.besttour.com.tw/"]');
                foreach ($descImages as $key => $descImage) {
                    preg_match('~^https://edgio-cdn-pix.besttour.com.tw/(.*)~', $descImage->src, $matchSubUrl);
