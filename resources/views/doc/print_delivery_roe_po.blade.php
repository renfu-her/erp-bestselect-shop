            ...
            <table width="710" cellpadding="5"
                ...
                @foreach($delivery->delivery_items as $db_value)
                    <tr>
                        ...
                        <td>{{ $delivery->po_memo }} <a href="{{ $delivery->po_source_link }}">{{ $delivery->delivery_event_sn }}</a> {{ $db_value->taxation == 1 ? '應稅' : '免稅' }} {!! nl2br($db_value->note) !!} {!! nl2br($db_value->po_note) !!}</td>
                    </tr>
                @endforeach
                @endif
                </tbody>
