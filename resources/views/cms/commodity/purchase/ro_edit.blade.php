...
@section('sub-content')
    ...
    <form method="POST" action="{{ $formAction }}" class="-banRedo">
        ...
        <div class="card shadow p-4 mb-4">
            ...
            <p class="fw-bold">客戶：{{ $purchaseData->supplier_name }}</p>
            ...
            <div class="table-responsive tableOverBox border-bottom border-dark">
                <table class="table table-sm table-hover tableList mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $ro_data->first() ? $ro_data->first()->sn : '' }}</td>
                            <td class="text-wrap">{{ $value->product_title }}{{'（' . $value->price . ' * ' . $value->qty . '）'}}</td>
        ...
        <div class="col-auto">
            ...
            <a href="{{ Route('cms.purchase.return_detail', ['return_id' => $return->id]) }}" class="btn btn-outline-primary px-4" role="button">
                返回明細
            </a>
        </div>
    </form>
