...
@section('sub-content')
    ...
    <form method="POST" action="{{ route('cms.product.edit-web-spec', ['id' => $product->id]) }}">
        @csrf
                    ...
                    <div class="mb-2 row sortabled_box -cloneElem">
                        <div class="col d-flex flex-column flex-sm-row pe-0">
                            <div class="col col-sm-5 col-lg-3 px-0 pb-2 pb-sm-0">
                                <input type="text" name="title[]" class="form-control" maxlength="10" value="{{  $list['title'] }}"
                                    placeholder="請輸入標題。例：材質" aria-label="規格說明標題">
                            </div>
                            <div class="col px-0 px-sm-2 pb-2 pb-sm-0">
                                <input type="text" name="content[]" value="{{ $list['content'] }}" class="form-control" placeholder="請輸入內文。例：棉 / 聚脂纖維"
                                    aria-label="規格說明內文" maxlength="300">
                            </div>
                        </div>
