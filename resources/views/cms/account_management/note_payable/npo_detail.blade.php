...
@section('sub-content')
    ...
    <div class="card shadow p-4 mb-4">
        <div class="row mb-3 border rounded mx-0 px-0 pt-2">
            ...
            <div class="card-body px-4 d-flex align-items-center bg-white flex-wrap justify-content-end">
                <strong class="flex-grow-1 mb-0">{{ request('qd') ? date('Y-m-d', strtotime(request('qd'))) : '' }} 兌現之應付票據清單</strong>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    @if($note_payable_order)
                        <tr>
                            ...
                            <td></td>
                            <td></td>
                            <td>{{ number_format($note_payable_order->amt_total_net) }}</td>
                        </tr>
                        @php
                            $d_count++;
                        @endphp
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($note_payable_order ? $note_payable_order->amt_total_net : 0) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
