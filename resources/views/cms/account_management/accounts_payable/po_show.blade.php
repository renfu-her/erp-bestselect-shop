    ...
    <nav class="col-12 border border-bottom-0 rounded-top nav-bg">
        <div class="p-1 pb-0">
            ...
            <a href="{{ route('cms.collection_payment.edit', ['id' => $paying_order->id]) }}" 
                class="btn btn-sm btn-success px-3 mb-1" role="button">更改付款對象</a>
            @endcan

            @can('cms.collection_payment.delete')
            @if(! $data_status_check)
                @if(! ($paying_order->payment_date && $paying_order->append_po_id))
                    <a href="{{ route('cms.collection_payment.payable_list', ['id' => $paying_order->id]) }}" 
                        class="btn btn-sm btn-primary mb-1" role="button">付款記錄</a>
            ...
            @can('cms.collection_payment.edit')
            <a href="{{ route('cms.ref_expenditure_petition.edit', ['current_sn' => $paying_order->sn]) }}" 
