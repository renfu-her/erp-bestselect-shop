                ...
                <div class="col-12 col-sm-6 mb-3">
                    ...
                    <select class="form-select -select2 -single" name="client_key" aria-label="客戶" data-placeholder="請選擇客戶" required>
                        ...
                        @foreach ($client as $value)
                            ...
                            @endphp
                            <option value="{{ $value['id'] . '|' . $value['name'] }}" {{ $value['id'] . '|' . $value['name'] == old('client_key', $received_order->drawee_id . '|' . $drawee_name[0]) ? 'selected' : '' }}>{{ $value['name'] . ' - ' . (isset($value['title']) ? $value['title'] . ' ' : '') . ($value['email'] ?? $value['id']) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
