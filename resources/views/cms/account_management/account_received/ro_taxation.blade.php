                    ...
                    <tbody>
                        ...
                        @foreach($order_list_data as $value)
                            <tr>
                                <td class="text-center">{{ $serial }}</td>
                                <td>
                                    <select class="form-select -select2 -single" name="product[{{ $value->received_id }}][grade_id]" data-placeholder="請選擇會計科目" required>
                                        <option value="" selected disabled>請選擇</option>
                                        @foreach($total_grades as $g_value)
                                            <option value="{{ $g_value['primary_id'] }}"{{ $g_value['primary_id'] == $value->ro_received_grade_id ? 'selected' : '' }}
                                                @if($g_value['grade_num'] === 1)
                                                    class="grade_1"
                                                @elseif($g_value['grade_num'] === 2)
                                                    class="grade_2"
                                ...
                                <td>
                                    ...
                                    <input class="form-control" name="product[{{ $value->received_id }}][summary]" type="text" value="{{ $value->summary }}">
                                </td>
                                <td>1</td>
                                <td>{{ number_format($value->tw_price, 2) }}</td>
                                <td>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" name="product[{{ $value->received_id }}][taxation]" value="1" type="radio" id="tax_{{ $serial }}_1" required @if ($value->taxation == '1') checked @endif>
                                        <label class="form-check-label" for="tax_{{ $serial }}_1">應稅</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" name="product[{{ $value->received_id }}][taxation]" value="0" type="radio" id="tax_{{ $serial }}_2" required @if ($value->taxation == '0') checked @endif>
                                        <label class="form-check-label" for="tax_{{ $serial }}_2">免稅</label>
                                    </div>
                                </td>
                                <td><input class="form-control" name="product[{{ $value->received_id }}][note]" type="text" value="{{ $value->note }}"></td>
                            </tr>
                            @php
                                $serial++;
                            @endphp
