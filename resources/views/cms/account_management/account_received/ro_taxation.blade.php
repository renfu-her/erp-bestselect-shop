                    ...
                    <tbody>
                        ...
                        @foreach($received_data as $value)
                            <tr class="bg-info">
                                ...
                                <td><input class="form-control form-control-sm -l" name="received[{{ $value->received_id }}][note]" type="text" value="{{ $value->note }}"></td>
                            </tr>
                            @php
                                $serial++;
                            @endphp
                        ...
                        @foreach($order_list_data as $value)
                            <tr>
                                <td class="text-center">{{ $serial }}</td>
                                <td>
                                    <select class="form-select form-select-sm -select2 -single" name="product[{{ $value->received_id }}][grade_id]" data-placeholder="請選擇會計科目" required>
                                        <option value="" selected disabled>請選擇</option>
                                        @foreach($total_grades as $g_value)
                                ...
                                <td>
                                    <input class="form-control form-control-sm -l" name="product[{{ $value->received_id }}][summary]" 
                                        type="text" value="{{ $value->summary }}">
                                </td>
                                <td class="text-end">1</td>
                                <td class="text-end">{{ number_format($value->tw_price, 2) }}</td>
                                <td>
                                    <div class="form-check form-check-inline lh-base">
                                        <label class="form-check-label" for="tax_{{ $serial }}_1">
                                            <input class="form-check-input" 
                                                name="product[{{ $value->received_id }}][taxation]" 
                                                value="1" type="radio" id="tax_{{ $serial }}_1" 
                                                required @if ($value->taxation == '1') checked @endif>
                                            應稅
                                        </label>
                                ...
                                <td><input class="form-control form-control-sm -l" name="product[{{ $value->received_id }}][note]" type="text" value="{{ $value->note }}"></td>
                            </tr>
                            @php
                                $serial++;
                            @endphp
