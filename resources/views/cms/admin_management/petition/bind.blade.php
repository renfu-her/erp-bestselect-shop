...
@section('sub-content')
    ...
    <form id="form1" method="post" action="{{ route('cms.reverse-bind-page', ['sn' => $sn, 'prev_url' => $prev_url]) }}">
            ...
            <div class="row">
                ...
                <div>
                    ...
                    <div class="row -appendClone mb-2">
                        @if (count($order) == 0 && count(old('n_order', [])) == 0)
                            <div class="input-group col-12 col-md-6 mb-2 -cloneElem">
                                <input class="form-control" type="text" name="n_order[]" placeholder="請填入相關單號"
                                    aria-label="相關單號">
                        ...
                        @foreach ($order as $key => $value)
                            <div class="input-group col-12 col-md-6 mb-2 -cloneElem">
                                <input class="form-control @error('order.' . $key) is-invalid @enderror" type="text"
                                    name="n_order[]" value="{{ $value->sn }}" placeholder="請填入相關單號" aria-label="相關單號"
                                    disabled>
                                <button class="btn btn-outline-secondary -del" type="button" data-bs-toggle="tooltip"
                                    title="刪除">
                                    <i class="bi bi-x-lg"></i>
                        ...
                        @foreach (old('n_order', []) as $key => $value)
                            <div class="input-group col-12 col-md-6 mb-2 -cloneElem">
                                <input class="form-control @error('n_order.' . $key) is-invalid @enderror" type="text"
                                    name="n_order[]" value="{{ $value }}" placeholder="請填入相關單號" aria-label="相關單號">
                                <button class="btn btn-outline-secondary -del" type="button" data-bs-toggle="tooltip"
                                    title="刪除">
                                    <i class="bi bi-x-lg"></i>
                                </button>
        ...
        <div class="col-auto">
            ...
            <a href="{{ $prev_url }}" class="btn btn-outline-primary px-4" role="button">返回列表</a>
        </div>
    </form>
