            ...
            <div class="row">
                <div class="col-12 col-sm-6 mb-3">
                    ...
                    <input class="form-control @error('title') is-invalid @enderror" type="text" name="title"
                        value="{{ old('title', $data->title ?? '') }}" placeholder="例：日本水蜜桃蘋果" required aria-label="水果名稱">
                </div>
                <div class="col-12 col-sm-6 mb-3">
                    <label class="form-label">副標題 <span class="text-danger">*</span></label>
                    <input class="form-control @error('sub_title') is-invalid @enderror" type="text" name="sub_title"
                        value="{{ old('sub_title', $data->sub_title ?? '') }}" placeholder="例：數量稀少、多汁爽脆的最佳口味" required aria-label="副標題">
                </div>
                <div class="col-12 col-sm-6 mb-3">
                    ...
                    <input class="form-control @error('place') is-invalid @enderror" type="text" name="place"
                        value="{{ old('place', $data->place ?? '') }}" placeholder="例：日本青森" required aria-label="產地">
                </div>
                <div class="col-12 col-sm-6 mb-3">
                    <label class="form-label">產季 <span class="text-danger">*</span></label>
                    <input class="form-control @error('season') is-invalid @enderror" type="text" name="season"
                        value="{{ old('season', $data->season ?? '') }}" placeholder="例：秋季 (10月)" required aria-label="產季">
                </div>
                <div class="col-12 mb-3">
                    <label class="form-label">圖片網址 <span class="text-danger">*</span></label>
                    <input class="form-control @error('pic') is-invalid @enderror" type="url" name="pic"
                        value="{{ old('pic', $data->pic ?? '') }}"
                        placeholder="例：https://www.bestselection.com.tw/activity/fruits/image/Fru_sea_05_03.jpg" required
                        aria-label="圖片網址">
                </div>
                <div class="col-12 mb-3">
                    ...
                    <input class="form-control @error('link') is-invalid @enderror" type="url" name="link"
                        ...
                        placeholder="例：https://www.bestselection.com.tw/product/P230508001?openExternalBrowser=1" required
                        aria-label="產品連結">
                </div>
                <div class="col-12 mb-3">
                    ...
                    <textarea name="text" class="form-control @error('text') is-invalid @enderror"
                        placeholder="例：外表黃綠外帶有一抹腮紅，因此又被暱稱為『水蜜桃蘋果』。" aria-label="內文">{{ old('text', $data->text ?? '') }}</textarea>
                </div>
                <div class="col-12 mb-3">
                    ...
                    <select name="status" class="form-select" required>
                        ...
                        @foreach ($saleStatus as $key => $status)
                            <option value="{{ $status }}" @class([
                                ...
                                'bg-danger' => $key == 13,
                                'bg-warning' => $key == 14,
                                'text-white' => $key == 0 || $key == 13,
                            ]) @if($status == old('status',$data->status ?? '')) selected @endif>
                                {{ $status }}</option>
                        @endforeach
                    </select>
                </div>
